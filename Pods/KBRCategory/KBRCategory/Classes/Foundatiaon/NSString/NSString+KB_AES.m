...
#import "NSString+KB_AES.h"
#import "NSData+KB_AES.h"

@implementation NSString (KB_AES)
...
- (NSString *) kb_AES256_Encrypt:(NSString *)key withIv:(NSString *)iv
{
    ...
    //对数据进行加密
    NSData *result = [data kb_AES256_Encrypt:key withIv:iv];
    //转换为2进制字符串
...
- (NSString *) kb_AES256_Decrypt:(NSString *)key withIv:(NSString *)iv
{
    ...
    //对数据进行解密
    NSData* result = [data kb_AES256_Decrypt:key withIv:iv];
    if (result && result.length > 0)
    {
