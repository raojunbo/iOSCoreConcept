...
- (UIColor *)colorByAddingBrightness:(CGFloat)byBrightness
{
	CGFloat hue, saturation, brightness, alpha;
	
	const BOOL convertOK = [self getHue:&hue saturation:&saturation brightness:&brightness alpha:&alpha];
	if (convertOK) {
		return [UIColor colorWithHue:hue saturation:saturation brightness:brightness + byBrightness alpha:alpha];
	} else {
		return self;
	}
}
...
{
	return [self colorByAddingBrightness:-0.05];
}
