...
- (void)_mapFromJSON:(NSDictionary *)jsonDictionary
  ...
  _layerName = [jsonDictionary[@"nm"] copy];//名字
  _layerID = [jsonDictionary[@"ind"] copy];//层标识
  
  NSNumber *layerType = jsonDictionary[@"ty"];//layerType
  _layerType = layerType.integerValue;
  
  if (jsonDictionary[@"refId"]) {//资源参考id
    _referenceID = [jsonDictionary[@"refId"] copy];
  }
  
  _parentID = [jsonDictionary[@"parent"] copy];//父图层
  
  if (jsonDictionary[@"st"]) {
    _startFrame = [jsonDictionary[@"st"] copy];//开始帧
  }
  ...
  NSDictionary *anchor = ks[@"a"];//anchor
  if (anchor) {
